...
#OLD SECTION MAYBE SHOULD BE REMOVED (test=2)
if [ $TEST = 2 ]; then
...
echo '<table width="960">'
echo '    <tr class="even">'
echo '      <td width="960"><p><h2>Audio tweaks(OTG-speed, FIQ-split accelerations and ALSA output level):</h2></p></td>'
echo '    </tr>'

echo '<table width="960">'
echo '  <form name="setcmd" form action="writetocmd.cgi" method="get">'
echo '    <tr class="odd">'
echo '      <td width="150"><p>OTG-speed</p></td>'
echo '      <td width="150"><input type="radio" name="CMD" id="not_enabled" value="Default" '$CMDdefault'>Default</td>'
echo '      <td width="800"><p>Default speed - if sound is OK then keep default</p></td>'
echo '    </tr>'
echo '    <tr class="even">'
echo '      <td width="150"><p></p></td>'
echo '      <td width="150"><input type="radio" name="CMD" id="enabled" value="Slow" '$CMDslow'>dwc_otg.speed=1</td>'
echo '      <td width="800"><p>Add dwc_otg.speed=1 to cmdline.txt - often needed for C-Media based DACs, try if sound is crackling'
echo '                         Reboot is needed.</p></td>'
echo '    </tr>'
echo '    <tr class="odd">'
...
#NEW SECTION STARTS - THIS CAN BE DELETED IF IN TROUBLE
if [ $TEST = 1 ]; then
...
echo '     <table class="cfgframe" cellspacing="2" cellpadding="0" width="100%" align="center">'
echo '      <tr>'
echo '       <td colspan="2" class="header"><nobr>AUDIO TWEAKS.</nobr></td></tr>'
echo '      <tr>'
...
echo ' 	<br />'
#echo ' <span style="color:black;" title="Use alsamixer via SHH, and save your custom settings by sudo amixer store">Use only if you have changed ALSA output level via ALSA-mixer, allows custom output level after reboot</span>'
...
case $ALSAlevelout in
	Default)
		echo '<p class="info">Default - ALSA output level is used<br /><br /></p>'
		;;
	Custom)
		echo '<p class="info">Custom - ALSA output level will be defined from your custom settings.<br /><br /></p>'
		;;
...
case $FIQ in
	0x1)
		echo '<p class="info">FIQ-split value is: '$FIQ'<br /><br />'
...
#OLD SECTION MIGHT BE REMOVED ONLY SHOW IF TEST=2
if [ $TEST = 2 ]; then
...
echo '<table border="0" width="960">'
echo '  <form name="fiq" action= "writetofiq.cgi" method="get">'
echo '    <tr class="odd">'
echo '      <td width="150"><p>FIQ-split</p></td>'
echo '      <td width="150">'
...
echo '		<option value="0x8" '$selected6'> 6. Enable Interrupt/Control Split Transaction hack - Value 0x8 </option><br />'
echo '      </td>'
echo '      <td width="800"><p>Change FIQ_FSM USB settings. This might solve USB audio problems.'
echo '                         Important for specific USB-DACs - like the Naim DAC-V1 card, try option 1, 2 or 3.'
echo '                         Reboot is needed.</p></td>'
echo '    </tr>'
echo '    <tr class="even">'
...
echo '<table width="960">'
echo '  <form name="setcmd" form action="writetoALSAlevelout.cgi" method="get">'
echo '    <tr class="odd">'
echo '      <td width="150"><p>ALSA-output level</p></td>'
echo '      <td width="150"><input type="radio" name="ALSAlevelout" id="Default" value="Default" '$ALSAdefault'>Default</td>'
echo '      <td width="800"><p>Default is to use maximal output level - allows bitperfect audio out, and will remove your custom ALSA settings file</p></td>'
echo '    </tr>'
echo '    <tr class="even">'
echo '      <td width="150"><p></p></td>'
echo '      <td width="150"><input type="radio" name="ALSAlevelout" id="Custom" value="Custom" '$ALSAcustom'>Custom</td>'
echo '      <td width="800"><p>Use if you have changed ALSA output level via ALSA-mixer, so your custom settings will be used after reboot.</p></td>'
echo '    </tr>'
echo '    <tr class="odd">'
echo '      <td colspan="3"><input type="submit" value="Submit"></td>'
echo '    </tr>'
...
case $ALSAlevelout in
	Default)
		echo '<p class="info">Default - ALSA output level is 100%<br /><br /></p>'
		;;
	Custom)
		echo '<p class="info">Custom - ALSA output level will be defined from your custom settings.<br /><br /></p>'
		;;
