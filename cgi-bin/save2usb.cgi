...
is_mounted() {
	...
	if [ -f /mnt/sda1/newconfig.cfg ]; then
		...
		echo '<p>Note: If you boot with this USB-stick attached then this file will be copied by piCorePlayer and used as config file.
			     This is handy if you update your piCorePlayer or want to setup another piCorePlayer with similar settings.</p>'
	else
		echo '<h2>Something went wrong! Your config file (config.cfg) was not saved - please reboot with your USB attached and then try to save your config file again.</h2>'
	fi
	sync
...
echo '  <meta http-equiv="Refresh" content="30; url=main.cgi">'
...
if mount | grep $VOLUME_A; then
	...
	echo '<p class="error">[ ERROR ] '$VOLUME_A' is NOT mounted.</p>'
