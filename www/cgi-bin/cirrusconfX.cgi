...
pcp_install_cirrus() {
	pcp_message INFO "Downloading Cirrus Configuration extension..." "html"
	sudo -u tc pcp-load -r $PCP_REPO -w rpi-cirrus-config.tcz
	if [ -f $TCEMNT/tce/optional/rpi-cirrus-config.tcz ]; then
		pcp_message INFO "Installing Cirrus Configuration extension..." "html"
		...
		[ $DEBUG -eq 1 ] && pcp_message DEBUG "rpi-cirrus-config.tcz is added to onboot.lst" "html"
...
pcp_remove_cirrus() {
	...
	sudo sed -i '/rpi-cirrus-config.tcz/d' $ONBOOTLST
	echo "Removing configuration files" "html"
	rm -f $CIRRUSCONF
}
...
set_cirrus_conf() {
	...
	if [ x"$SPEAKERS" = "x" ]; then
		pcp_message INFO "Setting Speakers OFF" "html"
		SPEAKERS=0
		reset_speaker_out
	else
		pcp_message INFO "Setting Speakers ON" "html"
		playback_to_speakers
	fi
	if [ x"$HEADSET" = "x" ]; then
		pcp_message INFO "Setting Headset OFF" "html"
		HEADSET=0
		reset_headset_out
	else
		pcp_message INFO "Setting Headset ON" "html"
		playback_to_headset
	fi
	if [ x"$LINEOUT" = "x" ]; then
		pcp_message INFO "Setting Line Out OFF" "html"
		LINEOUT=0
		reset_line_out
	else
		pcp_message INFO "Setting Line Out ON" "html"
		playback_to_lineout
	fi
	if [ x"$SPDIF" = "x" ]; then
		pcp_message INFO "Setting Spdif OFF" "html"
		SPDIF=0
		reset_spdif_out
	else
		pcp_message INFO "Setting Spdif ON" "html"
		playback_to_spdif
	fi
